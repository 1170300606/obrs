...
type GenesisDoc struct {
	...
	InitialSlot   LTime              `json:"initial_slot"`
	Validators    []GenesisValidator `json:"validators,omitempty"`
	PubValidator  GenesisValidator   `json:"public_validator"`
	SupportQuorum Quorum             `json:"supprot_quorum,omitempty"`
...
func (genDoc *GenesisDoc) ValidateAndComplete() error {
	...
	if genDoc.InitialSlot < 0 {
		return fmt.Errorf("initial_height cannot be negative (got %v)", genDoc.InitialSlot)
	}
...
func (genDoc *GenesisDoc) ValidatorSet() *ValidatorSet {
	...
	for i, v := range genDoc.Validators {
		vals[i] = NewValidator(v.PubKey)
	}
	vset := NewValidatorSet(vals)
	return vset
}

func (GenDoc *GenesisDoc) PublicValidator() *Validator {
	pub_val := NewValidator(GenDoc.PubValidator.PubKey)
	return pub_val
}
